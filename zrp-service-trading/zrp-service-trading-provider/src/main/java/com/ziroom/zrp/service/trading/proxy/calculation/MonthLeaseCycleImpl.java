 ...
 * @since 1.0
 */
public class MonthLeaseCycleImpl implements LeaseCycle {
	private static final Logger LOGGER = LoggerFactory.getLogger(MonthLeaseCycleImpl.class);
	private RentalRate rentalRate;

	public MonthLeaseCycleImpl(LeaseCyclePojo leaseCyclePojo) throws Exception{
	...
	public PaymentTermsDto calculate(PaymentBaseDataPojo paymentBaseDataPojo) throws Exception{
		...
		// 判断租期
		if (conRentYear <= 3) { //1-3月
			actualRentPrice = new BigDecimal(rentalRate.rentPriceStrategyEntity.getShortPriceRate() * roomSalesPrice).setScale(0,BigDecimal.ROUND_HALF_UP);
			...
			actualRentPrice = new BigDecimal(rentalRate.rentPriceStrategyEntity.getShortPrice2Rate() * roomSalesPrice).setScale(0,BigDecimal.ROUND_HALF_UP);
			totalRentPrice = actualRentPrice.multiply(new BigDecimal(conRentYear));
			servicePrice = totalRentPrice.multiply(new BigDecimal(rentalRate.rentPriceStrategyEntity.getShortSPrice2Rate())).multiply(rentalRate.getStandardServiceRate()).setScale(0,BigDecimal.ROUND_HALF_UP);
		}
		...
		paymentTermsDto.setRenewDepositPriceDiff(renewDepositPriceDiff);// 续约押金差
