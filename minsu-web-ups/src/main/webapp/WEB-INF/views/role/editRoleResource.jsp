        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    <input id="roleFid" value="${roleEntity.fid }" type="hidden" >
                	<div class="ibox-content">
			           <form class="form-horizontal m-t" id="commentForm">
					        <div class="form-group">
					           <label class="col-sm-1 control-label">选择系统：</label>
	                           <div class="col-sm-2">
                                   <select class="form-control m-b" name="systemsFid" disabled="disabled" id="systemFid">
                                       <option value="">请选择</option>
                                       <c:forEach items="${sysList }" var="sys">
                                       <option value="${sys.fid }" ${roleEntity.systemsFid==sys.fid?'selected':'' }>${sys.sysName }</option>
                                       </c:forEach>
                                   </select>
                               </div>
			                   <label class="col-sm-1 control-label">角色名称：</label>
			                   <div class="col-sm-2">
			                       <input id="roleName" name="roleName" type="text" class="form-control" value="${roleEntity.roleName }"  readonly>
			                   </div>
					           <label class="col-sm-1 control-label">角色类型：</label>
			                   <div class="col-sm-2">
			                   	   <select class="form-control m-b" name="roleType"  id="roleType">
			                   	   		<option value="0"  ${roleEntity.roleType==0?'selected':'' }>普通角色</option>
			                   	   		<option value="1" ${roleEntity.roleType==1?'selected':'' }>数据区域角色</option>
			                   	   		<option value="2" ${roleEntity.roleType==2?'selected':'' }>区域角色</option>
			                   	   		<option value="3" ${roleEntity.roleType==3?'selected':'' }>数据角色</option>
			                   	   </select>
			                   </div>
					           <div class="col-sm-1">
