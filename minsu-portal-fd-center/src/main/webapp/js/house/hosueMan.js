(function ($) {
				...
				if(data.code == 0){
					...
					for (var i = 0; i < houseList.length; i++) {
						...
						if(status ==11){
							...
							_html +="<a href='"+localhostPaht+"/house/"+houseFid+"/goIssue\'><h1>"+(houseList[i].name == null ? "未完成发布的美居":houseList[i].name)+"</h1></a>";
						}
	...
	houseMan.getOptionByHouseStatu =  function(status,houseFid,roomFid,rentWay){
	   ...
	   case 11:
	   case 20:
		   // hosueHtml = "<div class='btns'><a class='btn active' onclick=\"HouseMan.cancle(\'"+houseFid+"\',\'"+(roomFid == null?'':roomFid)+"\',"+rentWay+")\">取消发布</a></div>"
