...
/**
 * 自定义#block命令
 * 作用：可以定义一个快 不用时候直接用#override 覆盖掉
 * @author yd
...
public class BlockDirective extends Directive{
	...
	public boolean render(InternalContextAdapter context, Writer writer, Node node)
        ...
        if(overrideNode == null) {
        	...
        	VelocityUtils.setParentForTop(new OverrideNodeWrapper(topNode),overrideNode);
	...
	private OverrideNodeWrapper getOverrideNode(InternalContextAdapter context,String name) {
		return (OverrideNodeWrapper)context.get(VelocityUtils.getOverrideVariableName(name));
	}
