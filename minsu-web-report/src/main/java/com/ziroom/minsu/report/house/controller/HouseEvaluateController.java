...
public class HouseEvaluateController {
    /**
     * 日志对象
     */
    private static final Logger LOGGER = LoggerFactory.getLogger(HouseEvaluateController.class);
     ...
     * @return
     */
    @RequestMapping("/toEntireHouseEvaluate")
    public ModelAndView toEntireHouseOrderLifeCycle(HttpServletRequest request) {
        ModelAndView maView = new ModelAndView("/house/houseEvaluate/entireHouseEvaluate");
        //国家下拉列表
     ...
     * @return
     */
    @RequestMapping("/toSubHouseEvaluate")
    public ModelAndView toSubHouseOrderLifeCycle(HttpServletRequest request) {
        ModelAndView maView = new ModelAndView("/house/houseEvaluate/subHouseEvaluate");
    ...
    @ResponseBody
    public PagingResult houseEvaluate(HouseEvaluateRequest paramRequest, HttpServletRequest request) {
        LogUtil.info(LOGGER, "houseEvaluate param:{}", JsonEntityTransform.Object2Json(paramRequest));
        PagingResult result = new PagingResult();
        try {
            result = houseDetailService.getHouseEvaInfo(paramRequest);
        } catch (Exception ex) {
            LogUtil.error(LOGGER, "houseEvaluate param:{},error:{}", JsonEntityTransform.Object2Json(paramRequest), ex);
        }
     ...
     * @return
     */
    @RequestMapping("/houseEvaluateExcelList")
    public void houseEvaluateExcelList(HouseEvaluateRequest paramRequest, HttpServletResponse response) {
        DealExcelUtil test = new DealExcelUtil(houseDetailService, paramRequest, null, "houseEvaluate-oper");
