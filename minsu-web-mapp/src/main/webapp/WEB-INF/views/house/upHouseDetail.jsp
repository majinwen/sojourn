...
<body>
		...
		<div class="bodys">
			<div id="main" class="main">
				...
				<ul id="houseList" class="myCenterList">
					<c:if test="${rentWay == 0 }">
							...
							<li 
							<c:if test="${houseBaseDetailVo.houseStatus == 11 || houseBaseDetailVo.houseStatus == 40 }">
								<c:if test="${!empty houseBaseDetailVo.houseArea && houseBaseDetailVo.houseArea>0  }">class="c_ipt_li bor_b"</c:if>
								...
								<a href="javascript:void(0);"> 
									...
									<input class="c_ipt c_disabled" data-type="area" type="tel" placeholder="请填写整套房源面积" id="houseArea" value="${houseBaseDetailVo.houseArea }m²" maxlength="6" readonly="readonly">
								</a>
							</c:if>
							<c:if test="${houseBaseDetailVo.houseStatus != 11 && houseBaseDetailVo.houseStatus != 40 }">
									<c:if test="${!empty houseBaseDetailVo.houseArea && houseBaseDetailVo.houseArea>0  }">class="c_ipt_li bor_b"</c:if>
					...
					<c:if test="${rentWay == 0 }">
						<%--<li <c:if test="${houseBaseDetailVo.leasePrice >0  }">class="c_ipt_li  bor_b"</c:if>
					<c:if test="${houseBaseDetailVo.leasePrice <=0  }">class="c_ipt_li c_ipt_li_none bor_b"</c:if> > 
							<a href="javascript:void(0);">
								<span class="c_span">每晚价格</span>
								
								 <c:if test="${houseBaseDetailVo.leasePrice >0 }">
								    <input class="c_ipt j_ipt" data-type="price" type="tel" placeholder="请填写每晚价格" id="housePrice" maxlength="5" value="${houseBaseDetailVo.leasePrice }元"/> 
								  </c:if>
								   <c:if test="${houseBaseDetailVo.leasePrice <=0 }">
								    <input class="c_ipt j_ipt" data-type="price" type="tel" placeholder="请填写每晚价格" id="housePrice" maxlength="5" value=""/> 
								  </c:if>
							...
							<c:when test="${houseBaseDetailVo.leasePrice ==0}">
								<li class="c_ipt_li c_ipt_li_none bor_b ">
									<a href="javascript:void(0);" onclick="toPriceDetail()">
										...
										<input class="c_ipt"  type="tel" placeholder="请设置价格"  name="" maxlength="5" value="" readonly="readonly" />
										<span class="icon_r icon"></span>
									</a>
								</li>
							</c:when>
			...
			<div class="box box_bottom">
				...
				<input type="button" id="sentHouse" value="下一步" class="org_btn">
				 -->
				<input id="nextBtn" type="button" value="确认" class="org_btn">
			</div>
	</form>
	...
	<script src="${staticResourceUrl}/js/house/addHouseFour.js${VERSION}008 "></script>
	<script type="text/javascript">
		function toPriceDetail(){
			...
			if(isNullOrBlank(houseBaseFid)){
				...
				$("#submitBtnarroundInfo").removeAttr("disabled");
				return;
			}
			var url = "${basePath}houseIssue/${loginUnauth}/toPriceDetail?houseBaseFid=${houseBaseFid}&flag=${flag}&houseRoomFid=${houseRoomFid }&rentWay=0";
			jumpPage(url);
		}
	...
	function keepFloat(obj, float, len) {
		...
		// forAndroid
		function goMyHouse() {
			var url = '${basePath}houseInput/${loginUnauth}/goToHouseUpdate?houseBaseFid=${houseBaseFid}&rentWay=${rentWay}&houseRoomFid=${houseRoomFid}&flag=${flag}';
	...
	$(function(){
		...
		function jumpPage(url){
			...
			if(rentWay ==  0){
				...
				if($.trim(housePrice).length > 0 && $.trim(housePrice) != 0){
