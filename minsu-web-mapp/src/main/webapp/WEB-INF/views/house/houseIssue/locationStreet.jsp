...
<body>
	<form action="${basePath}houseDeploy/${loginUnauth}/toLocationFromOthers" method="get" id="form">
		...
		<div class="main myCenterListNoneA">
			...
			<ul class="myCenterList">
				<li class="clearfix">
					<input type="text" id="houseStreet" placeholder="请输入街道信息" name="houseStreet" class="ipt_lg" value="${houseStreet }">
				</li>
			</ul>
		</div>
	...
	<script type="text/javascript">
		$(function(){
			...
			$("#houseStreet").on('keyup change', function(){
				var houseStreet = $.trim($("#houseStreet").val());
				if(houseStreet.length > 100 ){
					$("#houseStreet").val(houseStreet.slice(0, 100));
				}
				if(houseStreet.length != 0){
					$("#submitBtn").removeClass("disabled_btn").addClass("org_btn");
				}else{
