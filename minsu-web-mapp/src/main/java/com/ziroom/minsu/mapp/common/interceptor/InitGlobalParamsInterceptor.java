...
public class InitGlobalParamsInterceptor extends HandlerInterceptorAdapter{
    ...
    private static final String picSuffix = ".jpg";
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if(!Check.NuNStr(basePath)&&!basePath.contains("https")){
        	basePathHttps = basePath.replace("http", "https");
        }

        LogUtil.info(log, "basePath={},basePathHttps={}",basePath, basePathHttps);
        if(Check.NuNStr(basePathHttps)){
        	basePathHttps = basePath;
        }
        if(basePathHttps.contains(":80")){
    		basePathHttps = basePathHttps.replace(":80", "");
    	}

        request.setAttribute("basePath", basePath);
        request.setAttribute("basePathHttps", basePathHttps);
		//log.info("当前项目静态资源地址staticResourceUrl=["+staticResourceUrl+"],服务器路径SERVER_HOST=["+serverHost+"],上下文路径path=["+serverContext+"],项目请求路径basePath=["+basePath+"],默认图片大小picSize=["+picSize+"]");
		//CDN的静态地址
        ...
        //图片默认后缀
        request.setAttribute("picSuffix", picSuffix);
