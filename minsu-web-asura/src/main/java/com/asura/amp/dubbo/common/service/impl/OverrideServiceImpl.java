...
import com.asura.amp.dubbo.common.entity.Pair;
import com.asura.amp.dubbo.common.service.OverrideService;
import com.asura.amp.dubbo.common.utils.SyncUtils;
...
public class OverrideServiceImpl extends AbstractService implements OverrideService{
    ...
    public void saveOverride(DynamicConfig override) {
        URL url = getUrlFromOverride(override);
        registryService.register(url);
    }

    public void updateOverride(DynamicConfig override) {
    ...
    public List<DynamicConfig> findByAddress(String address) {
        return  SyncUtils.url2OverrideList(findOverrideUrl(null, address, null));
    }

    public List<DynamicConfig> findByServiceAndAddress(String service, String address) {
        return  SyncUtils.url2OverrideList(findOverrideUrl(service, address, null));
    }
    
    public List<DynamicConfig> findByApplication(String application) {
        return SyncUtils.url2OverrideList(findOverrideUrl(null, null, application));
    }

    public List<DynamicConfig> findByService(String service) {
        return  SyncUtils.url2OverrideList(findOverrideUrl(service, null, null));
    }

    public List<DynamicConfig> findByServiceAndApplication(String service, String application) {
    ...
    public DynamicConfig findById(Long id) {
        return SyncUtils.url2Override(findOverrideUrlPair(id));
    }
    
    private URL getUrlFromOverride(DynamicConfig override) {
