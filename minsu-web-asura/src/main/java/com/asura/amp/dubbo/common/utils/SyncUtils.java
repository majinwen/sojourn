...
public class SyncUtils {
    ...
    public static DynamicConfig url2Override(Pair<Long, URL> pair) {
    	if (pair == null) {
    		return null;
    	}
        ...
        DynamicConfig o = new DynamicConfig();
        o.setId(id);
    ...
    public static List<DynamicConfig> url2OverrideList(Map<Long, URL> cs) {
        List<DynamicConfig> list = new ArrayList<DynamicConfig>();
        if(cs == null) return list;
        for(Map.Entry<Long, URL> entry : cs.entrySet()) {
        ...
        for(Map.Entry<Long, URL> entry : from.entrySet()) {
            ...
            for(Map.Entry<String, String> e : filter.entrySet()) {
                ...
                if(ADDRESS_FILTER_KEY.equals(key)) {
                	// add by zhangshaobin 2013.1.24 ==============================================================>
                	if(Constants.ROUTE_PROTOCOL.equals(url.getProtocol())) {
                		String rule = url.getParameterAndDecoded(Constants.RULE_KEY);
                		try {
                			RouteRule routeRule = RouteRule.parse(rule);
                			Set<String> ruleHosts = null;
                			if(value.indexOf(":") > 0) { // 带端口为提供者
                				ruleHosts = routeRule.getThenCondition().get("provider.host").getMatches();
                				value = value.substring(0, value.indexOf(":"));
                			} else {
                				ruleHosts = routeRule.getWhenCondition().get("consumer.host").getMatches();
                			}
                			
                			for(String host : ruleHosts) {
                				if(!"*".equals(host) && !value.equals(host) && !value.matches(host)) {
                					match = false;
                                    break;
                				}
                			}
