...
<script type="text/javascript">
	function editAppDynamicAjaxDone(json) {
...
<div class="pageContent">
	<form class="required-validate" action="servicecenter/application/dynamic/edit.do" method="post" onsubmit="return validateCallback(this, editAppDynamicAjaxDone)">
		<input type="hidden" name="id" value="${override.id}">
		<input type="hidden" name="application" value="${override.application}">
		<div class="pageFormContent" layoutH="56">
			<div class="panel" defH="130">
				...
				<div>
					<p style="width:100%;">
						<label>服务名：</label>
						<input type="text" id="application_service" name="service" value="${override.service}" class="required" size="50" />
						<select style="margin-left:5px;" onchange="addAppServiceValue(this)">
							<option value="">请选择</option>
							<c:forEach var="service" items="${serviceMap}" varStatus="status">
								<option value="${service.value}" ${override.service eq service.value ? 'selected="selected"':''}>${service.key}</option>
							</c:forEach>
						</select>
					</p>
					<p style="width:100%;">
						<label>消费者应用名：</label>
						<span>${override.application}</span>
					</p>
					<p style="width:100%;">
