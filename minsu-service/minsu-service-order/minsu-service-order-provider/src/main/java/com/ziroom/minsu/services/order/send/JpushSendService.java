 ...
 * @since 1.0
 */
@Service(value = "order.jpushSendService")
public class JpushSendService implements SendService<String, Integer, Map<String, String>,Map<String, String>> {
    ...
    @Override
    public boolean send(String uid, Integer template, Map<String, String> par,Map<String, String> bizPar) {
        boolean flag = false;
        try {
            if(Check.NuNObjs(uid,template)){
                LogUtil.error(LOGGER,"par is empty uid:{},template:{},par:{}",uid,template,par);
                return false;
            }
            //调用短信模板
            ...
            LogUtil.info(LOGGER, "开始发送推送 参数：uid:{},jpushConfig:{}，bizpar:{}", uid, JsonEntityTransform.Object2Json(jpushConfig), JsonEntityTransform.Object2Json(par));
            JpushUtils.sendPushOne(uid,jpushConfig,par);
            flag = true;
        }catch (Exception e){
