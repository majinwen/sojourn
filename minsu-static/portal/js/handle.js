...
window.ShearPhoto.MINGGE(function() {
	//██████████重要设置████████████████
	var relativeUrl = "/customer"; //你不要在后面加斜杠，系统会自动给你加上斜杠，不信看下面！   index.html的JS引用路径自己改，很简单的说
	/*
	...
	Shear.config({
		...
		preview: [100,60,30], // 开启动态预览图片 (数组成员整数型，禁止含小数点 可选false 和数组)     数组内是宽度设置，没有高度设！因为高度会按比例做事 ，此设置代表预览150 大小的预览图（你可以增加多个预览图,如[100,70,50]），设置越多预览图,shearphoto性能越差！官方不建意你开启这个功能，尽可能请设为preview:false

		/*记住 preview 尽量设false*/

		url: relativeUrl + "saveHeadImg", //后端处理地址，保证正确哦，这是常识，连这个地址都能写错，你就是菜B，已经在本版本中帮你加入相对路径，你基本不用改这里了
		
		urlIe:relativeUrl + "saveHeadImgIE",//兼容下ie吧

		scopeWidth: 440, //可拖动范围宽  也就是"main"对象的初始大小(整数型，禁止含小数点) 宽和高的值最好能一致  

		scopeHeight: 320, //可拖动范围高  也就是"main"对象的初始大小(整数型，禁止含小数点) 宽和高的值最好能一致      
	...
	Shear.complete = function(serverdata) { //截图成功完成时，由shearphoto.php返回数据过来的成功包
			...
			var creatImg;
			creatImg = document.getElementById("headImg");
			creatImg.src = serverdata.data["ImgUrl"];
	...
	var up = new ShearPhoto.frameUpImg({
		...
		url: relativeUrl + "uploadHeadImg", //HTML5切图时，不会用到该文件，后端处理地址，保证正确哦，这是常识，连这个地址都能写错，你就是菜B，已经在本版本中帮你加入相对路径，你基本不用改这里了
	...
	up.run(function(data, True) { //upload.php成功返回数据后
		...
		if (data === false) {
			Shear.SendUserMsg("错误：网络错误，请稍后重试", 5e3, 0, "#f4102b", "#fff", true, true);
			return;
		}
		if (data["erro"]) {
		...
		if(data.data){
			Shear.run(data.data["headUrl"], true);
			$("imgUuid").val(data.data["headUUid"]);
		} else {
	...
	Shear.addEvent(document.getElementById("saveShear"), "click", function() { //按下截图事件，提交到后端的shearphoto.php接收
		var _w = document.getElementById("smallbox").style.width;//图片宽度
		var _t = document.getElementById("movebox").style.top;//图片位置top
		var _l = document.getElementById("movebox").style.left;//图片位置left
		Shear.SendPHP({
			shearphoto: _w+','+_t+','+_l
