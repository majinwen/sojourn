...
window.ShearPhoto.prototype = {
	...
	preview: {
		...
		run: function(arg, thisMain) {
			...
			if (Object.prototype.toString.call(arg.preview) === "[object Array]") {
				var leng = arg.preview.length,
					EmptyFun = function() {},
					srcDefault = "http://minsustatic.d.ziroom.com/portal/images/house/default.gif";
				if (leng > 0) {
					arg.scope.parentNode.insertAdjacentHTML("afterEnd", '<ul id="preview" style="display:none;margin-left:8px;"></ul>');
					var HTML = "",
					...
					for (var i = 0; i < leng; i++) {
						...
						// HTML += '<li><dt style="width:' + arg.preview[i] + "px;height:" + (p ? Math.round(arg.preview[i] / p) + "px;" : "auto;") + ";background-color: #F5F5F5;border: " + borderW + "px solid #F5F5F5;" + '"><img src="' + srcDefault + '"/></dt><dd>'+arg.preview[i]+'X'+arg.preview[i]+'</dd></li>';
						HTML += '<li><dt style="width:' + arg.preview[i] + "px;height:" + (p ? Math.round(arg.preview[i] / p) + "px;" : "auto;") + ";background-color: #fafafa;border-radius: " + arg.preview[i]/2 + "px;" + '"><img src="' + srcDefault + '"/></dt><dd>'+arg.preview[i]+'X'+arg.preview[i]+'</dd></li>';
		...
		open_: function(arg) {
			...
			if (this.dom) {
				...
				this.dom.parentNode.style.width = "600px";
			} else {
	...
	run: function(ImgUrl, Trues, SetPro) {
		...
		image.onerror = function() {
			this_.pointhandle(0, 10, "无法读取图片。图片类型或路径不正确", 0, "#f82373", "#fff");
		};
	...
	again: function() {
		...
		this.arg.ImgDom.src = this.arg.ImgMain.src = "http://minsustatic.d.ziroom.com/portal/images/house/default.gif";
	},
		...
		this.SendUserMsg("正在为你处理截图，稍等...", 0, 2, "#fbeb61", "#3a414c",true,true, function() {
			...
			this_.MyAjax.carry({
				...
				success: function(serverdata) {
					...
					if (serverdata === false) {
						this_.SendUserMsg("错误：网络超时，请稍后重试！", 5e3, 0, "#f4102b", "#fff", false);
						return;
					}
					if (serverdata["erro"]) {
					...
					this_.pointhandle(2e3, 1, "头像截取成功", 2, "#fbeb61", "#3a414c");
	...
	Upsubmit: function(upfile) {
		try {
			...
			if (this.inArray(filestype.toLowerCase(), this.config.UpType) == -1) {
				this.DelCreateUpfile();
				this.config.erro("只支持JPG、PNG格式文件，请重新选择");
				return;
			}
			...
			this.DelCreateUpfile();
			this.config.erro("图片格式有误 或 图片路径有误，清重新选择");
			return;
		}
	},
	run: function(fun) {
