(function ($) {
	...
	userAuth.updateUserInfo = function(){
		...
		if($.trim(jieshao).length == 0){
			...
			return;
		}else if($.trim(jieshao).length>0&&$.trim(jieshao).length<70){
			$("#J_jieshaon_tishi").html("个人介绍不能少于70个字").show();
			return;
		}else{
	...
	// 校验证营业执照
	function isBussinessLicense(card) {  
	 var pattern = /^[a-zA-Z0-9]{1,30}$/;  
	 return pattern.test(card);  
	}  
	// 港澳通行证验证
	function isHKMacao(card) {  
	 var re = /^[HMhmcC]{1}([0-9]{10}|[0-9]{8})$/;
	 return re.test(card);  
	}  
	...
	// 台湾通行证验证
	function isGATidCard(card) {  
	 var re1 = /^[a-zA-Z0-9\/\()]{8,15}$/;  
	 return re1.test(card);
	} 
	...
	$("#authButton").click(function(){
		...
		var re = /^[0-9]*$/;  
		if(re.test($.trim(realName))){
			$('#J_name_tishi').html("姓名不能全为数字").show();
			return;
		}
		...
		if($.trim(handPicStr).length == 0 && $.trim(handFid).length == 0){
			if(idType==12){
				$("#J_zhaopian_tishi").html("请上传营业执照照片").show();
				return;
			}else{
