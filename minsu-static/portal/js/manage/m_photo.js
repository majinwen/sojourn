...
$(function(){
	...
	var forUrlJumpRoomFid = $("#roomFid").val()
    // 加载房源相册
    addHousePhotos();
    ...
    $('#sentHouseBtn').click(function(){
        ...
        if(rentWay == 0){
        	$.post("/houseIssue/release",{houseFid:housefid,rentWay:rentWay},function(data){
        		if(data.code == 0){
        ...
        if(rentWay == 1){
        	$.getJSON("/house/getFRooms",{houseFid:housefid, roomFid:forUrlJumpRoomFid, type:"fabu"},function(data){
        		if(data.code == 0){
...
$("#sentHouseList").delegate('li','click',function(){
  if($(this).hasClass("active")){
    ...
    $("#sureSentHouseBtn").click(function () {
        ...
        var params = {"houseFid": houseFid, rentWay: 1, "roomList": roomData}
        //合租发布房源
        $.post("/rooms/release", {param: JSON.stringify(params)}, function (data) {
            ...
            if (data.code == 0) {
                if (forUrlJumpRoomFid == null || forUrlJumpRoomFid == "" || forUrlJumpRoomFid == undefined) {
                    window.location.href = "/houseIssue/success/" + housefid + "?rfids=" + fids;
                } else {
                    window.location.href = "/houseIssue/success/" + housefid + "?rfids=" + fids + "&roomFid=" + forUrlJumpRoomFid;
                }
            } else {
                $("#sureSentHouseBtn").removeClass("disabled");
                showConfirm(data.msg, "确定");
    ...
    $('#lookHouseBtn').click(function(){
    	...
    	if(rentWay == 1){
    		$.getJSON("/house/getFRooms",{houseFid:housefid, roomFid:forUrlJumpRoomFid, type:"preview"},function(data){
    			if(data.code == 0){
...
function addHousePhotos(){
  $(".part_r_tt").text("真实照片");
	...
	//加载房源图片列表
	var photo_str = '';
	$.getJSON("/houseIssue/housePic/"+housefid+"?roomFid="+roomfid,function(data){
		var dataList = [];
		if(data.code == 0){
			dataList = data.data.picList;
		}else{
			dataList = [{"picType":1,"roomfid":"","url":""},{"picType":2,"roomfid":"","url":""},{"picType":3,"roomfid":"","url":""},{"picType":4,"roomfid":"","url":""}];
		}
		var roomNo  = 0;
...
function addPhotoList(){
  ...
  $.getJSON("/houseIssue/typepic?"+Math.random(),{houseFid:housefid,roomFid:roomfid,picType:roomType},function(data){
    if(data.code == 0){
...
function getFiles(e) {
  ...
  e = e || window.event;
    ...
    var len ;
    if(lenLi == maxImgLen){
     showConfirm('每个区域最多可上传'+maxImgLen+'张照片','确定');
     return;
   }else if((files.length+lenLi) > maxImgLen){
...
$(function(){
  $("#sureRemoveImageBtn").click(function(){
    ...
    if(status == 1){
      ...
      if(roomType == "1"){
        if(len <= 2){
          $("#delPhotoTxt").html('无法删除，客厅至少要保留2张图片');
          return;
        }
      }else if(roomType == "0" || roomType == "3"){
       if(len <= 1){
        $("#delPhotoTxt").html('无法删除，卧室/卫生间至少要保留1张图片');
        return;
      }
    }
  }
  ...
  var currentRoomFid = $("#roomFid").val();
  var param = {
    houseBaseFid:housefid,
    houseRoomFid:currentRoomFid,
    picType:roomType,
...
function subimage(){
  ...
  if(roomType == "1"){
    if(images < 2){
      showConfirm("客厅至少上传2张照片","确定");
    }else{
      toSubImage();
    }
  }else if(roomType == "3"){
    if(images < 1){
      showConfirm("卫生间至少上传1张照片","确定")
    }else{
      toSubImage();
    }
  }else if(roomType == "0" ){
	 if(images < 2){
	   showConfirm("卧室至少上传2张照片","确定")
	 }else{
	   toSubImage();
	 }
  }else{
