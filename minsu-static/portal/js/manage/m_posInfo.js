...
$(function(){
    ...
    if(lg=="" || la==""){
        ...
        var point = new BMap.Point(lg,la);
...
//            }
                ...
                var paramData={"houseType":$("#J_typeIpt").attr("data-type"),"rentWay":$("#J_chuzuType").val(),"operateSeq":$("#operateSeq").val(),
                		"nationCode":nowCodeArr[0],"cityCode":nowCodeArr[1],"areaCode":nowCodeArr[2],"communityName":oIpt2.val(),"longitude":$("#longitude").val(),
                		"latitude":$("#latitude").val(),"houseStreet":oIpt3.val(),"detailAddress":oIpt4.val(),"houseBaseFid":$("#houseFid").val(),"cityName":nowStrArr[1],"areaName":nowStrArr[2],"fid":$("#housePhyFid").val()};
                CommonUtils.ajaxPostSubmit("/houseIssue/insertHouseLocationMsg",paramData,function(data){
                	if(data.code==0){
                		...
                		if(roomFid==null||roomFid==""||roomFid==undefined){
               			     ...
               			     window.location.href="/houseIssue/basicMsg/"+data.data.houseBaseFid+"?roomFid="+roomFid;
               		     }
                	}else if(data.code == 2){
                		window.location.href = SSO_USER_LOGIN_URL+window.location.href;
                	}else{
                		//showShadedowTips(data.msg,2000);(showShadedowTips方法在当前页面显示是灰底，但是页面是白底，看不清到底提醒什么)
