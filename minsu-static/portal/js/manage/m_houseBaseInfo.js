        ...
        $('#J_next').click(function(){
            ...
            if(nowType=='whole'){
                ...
                if( ! /^[1-9][0-9]*$/.test($('#J_whole3 input').val())){
                    ...
                    return false;
                }else if(parseInt($('#J_whole3 input').val()) < parseInt(priceLow)){
                	$('#J_whole3 input').siblings('.J_tishi_txt').text("房源价格不能低于"+priceLow+"元");
                    $('#J_whole3 input').siblings('.J_tishi_txt').show();
                    return false;
                }else if(parseInt($('#J_whole3 input').val()) > parseInt(priceHigh)){
                	$('#J_whole3 input').siblings('.J_tishi_txt').text("房源价格不能高于"+priceHigh+"元");
                	$('#J_whole3 input').siblings('.J_tishi_txt').show();
                	return false;
                }else{
                    $('#J_whole3 input').siblings('.J_tishi_txt').hide();
                }
                var clef=Math.floor($('#J_whole3 input').val()*$("#cleaningFees").val());
                ...
                isClick = true;
                //提交更新整租房源信息
                var paramData={"fid":$("#houseFid").val(),"checkInLimit":$("#J_numIpt").attr("data-num"),"rentWay":$("#J_chuzuType").val(),
                ...
                CommonUtils.ajaxPostSubmit("/houseIssue/updateHouseBaseAndExt",paramData,function(data){
                	if(data.code==0){
                        ...
                        if(roomFid==null||roomFid==""||roomFid==undefined){
                        	...
                        	window.location.href="/houseIssue/configMsg/"+$("#houseFid").val()+"?roomFid="+roomFid;
                        }
                    } else if (data.code == 2) {
                        window.location.href = SSO_USER_LOGIN_URL + window.location.href;
                    } else {
