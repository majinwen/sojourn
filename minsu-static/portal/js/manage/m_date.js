...
function toExportDate () {
    ...
    // 信息提示
    if(houseStatus==40){
    	$("#exportMsg").html("保存并同步日历成功之后，我们每1个小时会为您同步一次日历");
    } else {
    	$("#exportMsg").html("房源上架后，我们每1小时会为您同步一次日历");
    }
...
function toExport(){
    ...
    if(houseStatus==40){
        var d = 1;
        var dst=self.setInterval(function(){
            d+=1;
            $("#toExport").val("同步中（"+d+"s）");
        },1000);
    }
    ...
    if(oldCalendarUrl && oldCalendarUrl==link){//恢复
            ...
            success: function (data) {
                if(data){
                    if (data.code == 0){
                        // 交互成功后
                        $("#toExport").hide();
                        $("#stopExport").removeClass("disabled").show();
                        $("#link").attr("disabled","disabled");
                        $("#relateStatus").val(0);
                        closeModalBox();
                        ...
                        $("#exportMsg").html("您的日历链接为无效链接,请检查链接地址或稍后再次尝试 。");
                    }
                }else{
                    // 交互成功或失败后
                    $("#toExport").removeClass("disabled");
        ...
        $.post("/house/saveHouseRelate",{"houseFid":houseFid,"roomFid":roomFid,"rentWay":rentWay,"calendarUrl":link,"houseStatus":houseStatus},function (data) {
            if (data.code == 0){
                // 交互成功后
                $("#toExport").hide();$("#stopExport").show();
                $("#link").attr("disabled","disabled");
...
function stopExport(){
    ...
    var fid = $.trim($("#relateFid").val());
    layer.confirm("为避免因纠纷而导致经济损失，建议您打开日历同步功能。请确认是否继续停止日历同步？", function(index){
        ...
        $.ajax({
            ...
            success: function (data) {
                if(data){
                    if (data.code == 0){
                        // 交互成功后
                        $("#exportMsg").html("停止同步日历成功，为避免因纠纷而导致经济损失，建议您打开日历同步功能 。");
                        $("#toExport").show();$("#stopExport").hide();
                        $("#link").removeAttr("disabled");
                        $("#relateStatus").val(1);
                        layer.close(index);
