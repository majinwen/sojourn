...
$(function(){
	...
	$("#reasonTxt").on('input',function(){
		...
		var len = $("#reasonList").find('li').length;
		var top = $("#reasonList").find('li:eq('+(len-2)+')').offset().top;
		setTimeout(function(){
...
function dateConflict(start,ent){
	layer.open({
		    ...
		    shadeClose: false,
		    yes: function(){
		      	window.location.href = SERVER_CONTEXT+"houseMgt/43e881/calendarDetail?backFalg=1&houseBaseFid="+$("#houseBaseFid").val()+"&houseRoomFid="+$("#houseRoomFid").val()+"&rentWay="+$("#rentWay").val();
		    }
		});
