	...
	lanOrder.showList = function (list,page){
            var _html="";
 			...
 			$.each(list,function(i,n){
                ...
                if (versionCode >= 100019) {
                    _html += "<a href='javascript:void(0);' onclick='toOrderDetail(\"" + n.orderSn + "\")' class='clearfix' >";
                } else {
...
//跳转到原生订单详情
function toOrderDetail(orderSn) {
    var sourceType = $("#sourceType").val();
    if (sourceType == "1") {
        //兼容之前版本处理(多加一个参数)
        window.WebViewFunc.toLandlordOrderDetail(orderSn);
    } else if (sourceType == "2") {
        toLandlordOrderDetail(orderSn);
    }
}