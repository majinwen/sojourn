...
(function ($) {
    ...
    commonUtils.getOrderStatu = function(value, row, index){
        ...
        if(value == 35){
            orderStatuStr =  "房东未确认超时取消";
        }
        if(value == 36){
            orderStatuStr =  "未确认取消";
        }
        if(value == 37){
            orderStatuStr =  "客服协商取消";
        }
        if(value == 38){
            orderStatuStr =  "房东申请取消";
        }
        if(value==40){
        ...
        if(value == 72){
            orderStatuStr =  "客服协商退房";
        }
        if(value == 73){
            orderStatuStr =  "房东申请退房";
        }
        if(orderStatuStr == ""){
            orderStatuStr =  "*";
        }
    ...
    commonUtils.getIncomeType= function(value, row, index){
        ...
        if(value == 6){
            incomeTypeStr= "用户清洁费佣金";
        }
        if(value == 7){
            incomeTypeStr= "付款单黑名单";
        }
        if(value == 8){
            incomeTypeStr= "罚款单";
        }
    ...
    commonUtils.getPaymentStatus=function(value, row, index){
		...
		if(value == 20){
	            paymentStatusStr= "打款失败待处理";
		}
		if(value == 21){
	            paymentStatusStr= "打款失败已处理";
		}
        if(value == 30){
            paymentStatusStr= "罚款冲抵取消";
        }
        if(value == 31){
            paymentStatusStr= "罚款冲抵重新生成取消";
        }
        if(value == 99){
            paymentStatusStr= "黑名单不打款";
        }
        if(paymentStatusStr == ""){
            paymentStatusStr = "*";
        }
    ...
    commonUtils.getPaySourceType=function(value, row, index){
		...
		if(value == 8){
            paySourceTypeStr= "清空优惠券金额";
		}
		if(value == 9){
            paySourceTypeStr= "打款失败重新生成";
		}
		if(value == 10){
            paySourceTypeStr= "清洁费";
		}
        if(value == 11){
            paySourceTypeStr= "返现";
        }
	...
	commonUtils.receiveType= function(value, row, index){
        if(value == 1){
           return "房东";
        }
        if(value == 2){
    ...
    // 格式化时间
    commonUtils.formateTimeStr = function(value, row, index) {
        if (value != null) {
    ...
    // 格式化时间
    commonUtils.formateDateStr = function(value, row, index) {
        if (value != null) {
            var vDate = new Date(value);
    ...
    //格式化CST日期的字串
    commonUtils.formatCSTDate = function(value){
      ...
      return commonUtils.formateDateTime(startTime);
    }
    
    //格式化CST日期的字串
    commonUtils.formatCSTDay = function(value){
	     ...
	     var strGMT = dateStr[0]+" "+dateStr[1]+" "+dateStr[2]+" "+dateStr[5]+" "+dateStr[3]+" GMT+0800";
	     var startTime = new Date(Date.parse(strGMT));
      return commonUtils.formateDay(startTime);
    }
    ...
    commonUtils.getOrderType=function(value, row, index){
        var payTypeStr= "";
        if(value == 1){
            payTypeStr= "立即预定";
        }
