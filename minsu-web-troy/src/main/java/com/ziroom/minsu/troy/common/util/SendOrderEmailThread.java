...
public class SendOrderEmailThread implements Runnable{
	...
	public void run() {
    	...
    	if(!Check.NuNObj(sendOrderEmailRequest.getCheckInDate())){
    		...
    		emailRequest.getParamsMap().put("{5}", DateUtil.dateFormat(sendOrderEmailRequest.getOrderStartDate(), "yyyy年MM月dd日"));
		}
    	if(!Check.NuNObj(sendOrderEmailRequest.getCheckOutDate())){
    		...
    		emailRequest.getParamsMap().put("{6}", DateUtil.dateFormat(sendOrderEmailRequest.getOrderEndDate(), "yyyy年MM月dd日"));
		}
