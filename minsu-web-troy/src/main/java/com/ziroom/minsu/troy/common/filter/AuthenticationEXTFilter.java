 ...
 */
package com.ziroom.minsu.troy.common.filter;
...
public class AuthenticationEXTFilter extends AbstractCasFilter {
    ...
    protected void initInternal(final FilterConfig filterConfig) throws ServletException {
        if (!isIgnoreInitConfiguration()) {
            ...
            String excludeStr = getPropertyFromInitParams(filterConfig, "excludes", null);            
            if (excludeStr!=null && excludeStr.trim().length()>0) {
    			String ex[] = excludeStr.trim().split(",");
    			excludes = new String[ex.length];
    			for (int i = 0; i < ex.length; i++) {
    				excludes[i] = ex[i].trim();
    			}
    		}
