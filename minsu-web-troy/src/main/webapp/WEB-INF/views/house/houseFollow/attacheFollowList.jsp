				...
				<div class="ibox-content">
					<form class="form-group">
						...
						<div class="row m-b">
							<div class="col-sm-1 text-right">
								<label class="control-label"
									style="height: 35px; line-height: 35px;">运营专员:</label>
							</div>
							<div class="col-sm-2">
							...
							<div class="col-sm-2">
								<select class="form-control" name="followStatus" id="followStatus">
									...
									<option value="103">客服未联系上房东</option>
									<option value="201">专员待跟进</option>
									<option value="202">专员跟进中</option>
								</select>
						    </div>
						    <div class="col-sm-1 text-right">
								<label class="control-label"
						...
						<div class="row m-b">
						  <!-- 审核未通过原因 -->
				  	        <div class="col-sm-1 text-right">
								<label class="control-label"style="height: 35px; line-height: 35px;">审核说明:</label>
							</div>
							<div class="col-sm-2">
...
<script type="text/javascript">
	...
	$(document).ready(function () {
	    $("#jqGrid").jqGrid({
	        url: 'house/houseFollow/attacheFollowData',
	        mtype: "post",
	        datatype: "json",
	        colModel: [
    ...
    function showLink(field, value, row){
        return "<a onclick='showDetail(\""+row.fid+"\","+row.rentWay+",\""+row.landordMobile+"\",\""+row.landlordUid+"\")' href='javascript:void(0);'>"+field+"</a>";
    }

    var showDetail =  function(followFid,rentWay,landordMobile,landlordUid){
		$.ajax({
            type: "POST",
            url: "house/houseFollow/lockAndSaveHouseFollow",
            dataType:"json",
            data: {"followFid":followFid,"rentWay":rentWay,"houseLockCode":"11002","customerMobile":landordMobile,"landlordUid":landlordUid},
            success: function (result) {
            	if(result.code==0){
            		$.openNewTab(new Date().getTime(),'house/houseFollow/attacheFollowDetail?followFid='+followFid+"&rentWay="+rentWay, "房源跟进");
            	}else{
