...
<body class="gray-bg">
						...
						<form id="form" class="form-horizontal">
							...
							<div class="row">
								<div class="form-group">
	                                ...
	                                <div class="col-sm-2">
	                                    <input id="surveyDate" name="surveyDate" class="laydate-icon form-control layer-date"/>
	                                </div>
	                                <label class="col-sm-1 control-label mtop">实勘专员:</label>
		                            <div class="col-sm-2">
	                                 	<input type="hidden" name="surveyEmpFid" value="${employee.fid }"/>
	                                 	<input type="text" class="form-control m-b" name="surveyEmpName" value="${employee.empName }" readonly="readonly"/>
	                                </div>
		                        </div>
							</div>
							<div class="row">
								<div class="form-group">
			                        ...
			                        <div class="col-sm-5">
			                            <textarea rows="3" name="remark" class="form-control m-b" onkeydown="calNum(this)" onkeyup="calNum(this)" onchange="calNum(this)"></textarea>
			                       	    <p class="textarea-desc-color">字数：<span>0</span><span>/500</span></p>
			                        </div>
		                        </div>
	                        </div>
							<div class="row">
								<customtag:authtag authUrl="house/houseSurvey/saveHouseSurvey">
	...
	<script>
		            ...
		            success: function (result) {
		            	if(result.code === 0){
		            		if(result.data.isZo){
								...
								layer.alert("只有该房源地推管家或维护管家才能上传实勘表", {icon: 5, title:'提示'});
		            		}
						} else {
	    ...
	    function saveHouseSurvey(){
	    	$.ajax({
	            type: "post",
	            url: "house/houseSurvey/saveHouseSurvey",
	            dataType:"json",
