...
<html>
  ...
  <body class="gray-bg">
       ...
       <div class="wrapper wrapper-content">
        <form action="house/houseTop/updateTopHouse" method="post" id="updateTopHouse">
	   	<input type="hidden"  name="fid"  value="${houseTopDetail.fid }" >
	    <input type="hidden"  name="houseBaseFid"  value="${houseTopDetail.houseBaseFid }" >
	   	<input type="hidden"  name="roomFid"  value="${houseTopDetail.roomFid }" >
	   	<input type="hidden"  name="rentWay"  value="${houseTopDetail.rentWay }" >
         <div class="row">
            <div class="col-sm-12">
                    ...
                    <div class="ibox-content">
                            ...
                            <div class="col-md-11">
                       	       <select class="form-control m-b" name="topTitle">
	                               <c:forEach items="${resList }" var="title">
	                               		<option value="${title.resContent }"  ${houseTopDetail.topTitle==title.resContent?'selected':'' }>${title.resContent }</option>
	                               </c:forEach>
                               </select>
                            </div>
                        </div>
                        ...
                        <div class="row show-grid">
                            ...
                            <div class="col-md-11">
                            	...
                            	<c:forEach items="${middlePicList }" var="mPic">
									<div class='picline' ><input type="radio" name="topMiddlePic"  value="${mPic.picUrl }"  ${houseTopDetail.topMiddlePic==mPic.picUrl?'checked':'' }><a href='${picBaseAddrMona }${mPic.picBaseUrl }${mPic.picSuffix}${picSize}${mPic.picSuffix}' title='主图' data-gallery='blueimp-gallery-main'><img src='${picBaseAddrMona }${mPic.picBaseUrl }${mPic.picSuffix}${picSize}${mPic.picSuffix}' style='height: 135px; width: 165px;'></a></div>
                            	</c:forEach>                            		
					            </div>
                            </div>
                        </div>
                        ...
                        <div class="row show-grid">
							...
							<div class="col-md-11"><input name="topShareTitle" value="${houseTopDetail.topShareTitle }"  type="text" class="form-control m-b" ></div>
                        </div>
                        <div class="row show-grid">
                        	<div class="col-md-1 text-center"><h4>亮点标题：</h4></div>
                        	<input type="hidden"  value="${houseTopDetail.sternTitle.fid }" name="sternTitleFid">
							<div class="col-md-11"><input name="sternTitle" value="${houseTopDetail.sternTitle.columnContent }"  type="text" class="form-control m-b" ></div>
                        </div>
                        <div class="row show-grid">
                        	<div class="col-md-1 text-center"><h4>亮点文本：</h4></div>
                        	<input type="hidden"  value="${houseTopDetail.sternContent.fid }" name="sternContentFid">
							<div class="col-md-11"><textarea id="sternContent"  name="sternContent" rows="8"  class="form-control m-b" >${houseTopDetail.sternContent.columnContent }</textarea></div>
                        </div>
                        <div class="row show-grid">
                        	<div class="col-md-1 text-center"><h4>封面图片：</h4><input type="hidden" id="coverPicParam" name="coverPicParam" value="${houseTopDetail.coverPicParam.picServerUuid }|${houseTopDetail.coverPicParam.picBaseUrl }|${houseTopDetail.coverPicParam.picSuffix }|${houseTopDetail.coverPicParam.width }|${houseTopDetail.coverPicParam.hight }"></div>
                        	<input type="hidden"  value="${houseTopDetail.coverPicParam.fid }" name="coverPicParamFid">
							<div class="col-md-10"  id="coverPic"><div class='picline' id='${houseTopDetail.coverPicParam.picServerUuid }'><a href='${picBaseAddrMona }${houseTopDetail.coverPicParam.picBaseUrl }${houseTopDetail.coverPicParam.picSuffix}${picSize}${houseTopDetail.coverPicParam.picSuffix}' title='图片' data-gallery='blueimp-gallery-main'><img src='${picBaseAddrMona }${houseTopDetail.coverPicParam.picBaseUrl }${houseTopDetail.coverPicParam.picSuffix}${picSize}${houseTopDetail.coverPicParam.picSuffix}' style='height: 135px; width: 165px;'></a></div></div>
                        ...
                        <div class="row show-grid">
                            <div class="col-md-12">
                                ...
                                <div id="cc" style="height: 500px; overflow: auto;">
	                                ...
	                                <c:forEach items="${houseTopDetail.topColumnList}" var="topC">
	                                
	                                	<li class='warning-element' id="${topC.fid }">
	                                		<div class='row show-grid'>
	                                			<div class='col-md-1'><h4>条目类型：</h4></div><div class='col-md-1'>
	                                			<c:if test="${topC.columnType==101}">
	                                			     亮点标题
	                                			</c:if>
	                                			<c:if test="${topC.columnType==102}">
	                                			     描述标题
	                                			</c:if>
	                                			<c:if test="${topC.columnType==103}">
	                                			     文章标题
	                                			</c:if>
	                                			<c:if test="${topC.columnType==104}">
	                                			     标题1
	                                			</c:if>
	                                			<c:if test="${topC.columnType==105}">
	                                			     标题2
	                                			</c:if>
	                                			<c:if test="${topC.columnType==201}">
	                                			     亮点文本
	                                			</c:if>
	                                			<c:if test="${topC.columnType==202}">
	                                			     文章文本
	                                			</c:if>
	                                			<c:if test="${topC.columnType==203}">
	                                			     图片注释
	                                			</c:if>
	                                			<c:if test="${topC.columnType==204}">
	                                			     文章出处
	                                			</c:if> 
	                                			<c:if test="${topC.columnType==301}">
	                                			     图片
	                                			</c:if>
	                                			<c:if test="${topC.columnType==302}">
	                                			     头图
	                                			</c:if> 
	                                			<c:if test="${topC.columnType==401}">
	                                			     视频
	                                			</c:if>                                			
	                                			<c:if test="${topC.columnType==501}">
	                                			     音频
	                                			</c:if>                                			
	                                			</div>
	                                			<div class='col-md-1'><h4>条目样式：</h4></div><div class='col-md-1'>
												<c:if test="${topC.columnStyle==101}">
	                                			     左对齐
	                                			</c:if> 
												<c:if test="${topC.columnStyle==102}">
	                                			    右对齐
	                                			</c:if>                                    			
												<c:if test="${topC.columnStyle==103}">
	                                			    中对齐
	                                			</c:if>                                    			
	                                			</div>
	                                			<c:if test="${topC.columnType==101||topC.columnType==102||topC.columnType==103||topC.columnType==104||topC.columnType==105 }">
	                                				<div class='col-md-1'><h4>标题内容：</h4></div><div class='col-md-2'><input  name='columnContentU' value='${topC.columnContent }'  type='text' class='form-control m-b' ></div>
	                                			</c:if>
	                                			<c:if test="${topC.columnType==201|| topC.columnType==202||topC.columnType==203||topC.columnType==204}">
	                                				<div class='col-md-1'><h4>文本内容：</h4></div><div class='col-md-2'><textarea name='columnContentU' rows='10'  class='form-control m-b'>${topC.columnContent }</textarea></div>
	                                			</c:if>
	                                			<c:if test="${ topC.columnType==301|| topC.columnType==302}">
	                                				<div class='col-md-1'><h4>图片：</h4></div><div class='col-md-2'><a href='${picBaseAddrMona }${topC.picBaseUrl }${topC.picSuffix}${picSize}${topC.picSuffix}' title='图片' data-gallery='blueimp-gallery-main'><img src='${picBaseAddrMona }${topC.picBaseUrl }${topC.picSuffix}${picSize}${topC.picSuffix}' style='height: 135px; width: 165px;'></a><input  name='columnContentU' value=' pic001' type='hidden' class='form-control m-b' ></div>
	                                			</c:if>
	                                			<c:if test="${ topC.columnType==401|| topC.columnType==501}">
	                                				<div class='col-md-1'><h4>文本内容：</h4></div><div class='col-md-2'><textarea name='columnContentU' rows='10'  class='form-control m-b'>${topC.columnContent }</textarea></div>
													<div class='col-md-1'><h4>图片：</h4></div><div class='col-md-2'><a href='${picBaseAddrMona }${topC.picBaseUrl }${topC.picSuffix}${picSize}${topC.picSuffix}' title='图片' data-gallery='blueimp-gallery-main'><img src='${picBaseAddrMona }${topC.picBaseUrl }${topC.picSuffix}${picSize}${topC.picSuffix}' style='height: 135px; width: 165px;'></a></div>
	                                			</c:if>
	                                			<input type="hidden"  name="houseTopColumnFidS" value="${topC.fid}">
	                                			<input type="hidden"  name="markUp" value="1">
	                                			<div class='col-md-1'><button type='button'  class='btn btn-primary' onclick="delTopColumn('${topC.fid}')"><span>删除条目</span></button></div>
	                                		</div>	
	                                	</li>	
	                                </c:forEach>
	                                </ul>
                        ...
                        <div class="row show-grid">
                            ...
                            <div class="col-md-1 text">
                            	<h4>文本内容：<font color="red">视频链家必须是腾讯视频分享链接</font></h4>
                            </div>
	   ...
	   <script src="js/minsu/common/topColumnWebuploader.js?${VERSION}004"></script>
       ...
       <script type="text/javascript">
		...
		$("#updateTopHouse").validate({
			...
			submitHandler:function(){
				...
				$.ajax({
		            type: "POST",
		            url: "house/houseTop/updateTopHouse",
		            dataType:"json",
		            data: $("#updateTopHouse").serialize(),
        ...
        $(document).ready(function () {
            ...
            $(".pic").hide();
            //标签修改选中
            var tagArray=$("#tagArray").val().split(",");
            $("input[name='tagFids']").each(function (i){
            	var tagFid=$(this).val().split("|")[0];
            	for(var i=0;i<tagArray.length;i++){
            		if(tagFid==tagArray[i]){
        ...
        function S4() {
       	...
       	//删除已有条目
       	function delTopColumn(fid){
			$.ajax({
	            type: "POST",
	            url: "house/houseTop/deleteTopHouseColumn",
	            dataType:"json",
	            data:{"fid":fid},
	            success: function (result) {
	            	if(result.code==0){
