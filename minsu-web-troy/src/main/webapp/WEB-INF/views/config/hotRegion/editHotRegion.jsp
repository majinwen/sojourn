  ...
  <body class="gray-bg">
               ...
               <div class="ibox float-e-margins">
			  	   <div class="ibox-content">
	                   <form class="form-group" id="form">
	                       <input name="fid" value="${hotRegion.fid }" type="hidden">
	                       <div class="row">
                               ...
                               <div class="col-sm-2">
                                   <select class="form-control m-b" name="regionType" id="regionType">
                                   	   <c:forEach items="${hotRegionEnumList }" var="obj">
	                                       <option value="${obj.key }" <c:if test="${hotRegion.regionType == obj.key }">selected</c:if>>${obj.text }</option>
                                   	   </c:forEach>
                                   </select>
                               </div>
			                   <label class="col-sm-1 control-label">名称:</label>
	                           <div class="col-sm-2">
                                   <input id="regionName" name="regionName" value="${hotRegion.regionName }" type="text" class="form-control">
                               </div>
			                   <label class="col-sm-1 control-label">获取中心坐标</label>
	                           <div class="col-sm-2">
			                       <input id="location" name="location" type="text" value="${hotRegion.longitude },${hotRegion.latitude }" class="form-control">
			                   </div>
			                   <div class="col-sm-3">
                                   <customtag:authtag authUrl="config/subway/toMap">
							  	   	   <button class="btn btn-primary" type="button" onclick="toBaiduMap();">没有获取坐标？去百度地图</button>
			                   	   </customtag:authtag>
							   </div>
	                       </div>
	                       <div class="row">
	                           ...
	                           <div class="col-sm-2">
                                   <input id="hot" name="hot" value="${hotRegion.hot }" type="text" class="form-control">
                               </div>
                               <label class="col-sm-1 control-label">半径(KM):</label>
	                           <div class="col-sm-2">
                                   <input id="radii" name="radii" value="${hotRegion.radii }" type="text" class="form-control">
                               </div>
                               <div class="col-sm-1">
                                   <customtag:authtag authUrl="config/hotRegion/updateHotRegion">
			                       	   <button class="btn btn-primary" id="editBtn" type="button" onclick="updateHotRegion();">保存</button>
			                   	   </customtag:authtag>
			                   </div>
	                       </div>
	                       <div class="row">
	   ...
	   <script>
	   		$(function(){
	   			...
	   			var options = {
   					geoJson	: geoJson,
   					nationCode: '${hotRegion.nationCode}',
   					provinceCode: '${hotRegion.provinceCode}',
   					cityCode: '${hotRegion.cityCode}'
	   			};
	   			geoCascade(options);
		    ...
		    function updateHotRegion(){
		    	...
		    	$.ajax({
					...
					url:"config/hotRegion/updateHotRegion",
					data:{
						"hotRegion":hotRegion
					},
					dataType:"json",
		    ...
		    function toJsonString(array){
				...
				$.each(array, function(i, n){
					...
					if(key == "location" && $.trim(value).length != 0){
						var arr = value.split(",");
						json["longitude"] = parseFloat(arr[0]);
						json["latitude"] = parseFloat(arr[1]);
						return true;
					}
