(function ($) {
    ...
    order.init = function(){
        //外部js调用
        laydate({
            elem: '#checkInTimeStart', //目标元素。由于laydate.js封装了一个轻量级的选择器引擎，因此elem还允许你传入class、tag但必须按照这种方式 '#id .class'
            event: 'focus' //响应事件。如果没有传入event，则按照默认的click
        });
        laydate({
            elem: '#checkInTimeEnd', //目标元素。由于laydate.js封装了一个轻量级的选择器引擎，因此elem还允许你传入class、tag但必须按照这种方式 '#id .class'
            event: 'focus' //响应事件。如果没有传入event，则按照默认的click
        });

        laydate({
            elem: '#checkOutTimeStart', //目标元素。由于laydate.js封装了一个轻量级的选择器引擎，因此elem还允许你传入class、tag但必须按照这种方式 '#id .class'
            event: 'focus' //响应事件。如果没有传入event，则按照默认的click
        });
        laydate({
            elem: '#checkOutTimeEnd', //目标元素。由于laydate.js封装了一个轻量级的选择器引擎，因此elem还允许你传入class、tag但必须按照这种方式 '#id .class'
            event: 'focus' //响应事件。如果没有传入event，则按照默认的click
        });
    ...
    order.paginationParam = function (params) {
    	return {
            ...
            checkInTimeStart:$("#checkInTimeStart").val(),
            checkInTimeEnd:$("#checkInTimeEnd").val(),
            checkOutTimeStart:$("#checkOutTimeStart").val(),
            checkOutTimeEnd:$("#checkOutTimeEnd").val(),
    ...
    order.getOrderDetail =  function(orderSn){
        ...
        $.openNewTab(new Date().getTime(),'order/getOrderDetail?orderSn='+orderSn, "新建优惠券活动");
